...
This code is poorly implemented.
We are in the process of modifying variables such as hostname and port to operate consistently.
Please refer to it when viewing.

이 코드는 구현이 미흡한 상태입니다.
hostname이나 port 등의 변수들이 일관성 있게 작동하도록 고치는 과정 중에 있습니다.
열람시 참조 바랍니다.
*/
...
int main(int argc,char **argv)
{
    ...
    listenfd = open_listenfd(argv[1]); // wrapper함수에서 시스템함수로 변경
    while(1){
        clientlen = sizeof(clientaddr);
        connfd = accept(listenfd,(SA *)&clientaddr,&clientlen);

        /*print accepted message*/
        getnameinfo((SA*)&clientaddr,clientlen,hostname,MAXLINE,port,MAXLINE,0); // 시스템함수로 변경
